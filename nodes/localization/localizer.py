#!/usr/bin/env python3

import math
import rospy

from tf.transformations import quaternion_from_euler
from tf2_ros import TransformBroadcaster
from pyproj import CRS, Transformer, Proj

from novatel_oem7_msgs.msg import INSPVA
from geometry_msgs.msg import PoseStamped, TwistStamped, Quaternion, TransformStamped

class Localizer:
    def __init__(self):
        # Parameters
        self.undulation = rospy.get_param('undulation')
        utm_origin_lat = rospy.get_param('utm_origin_lat')
        utm_origin_lon = rospy.get_param('utm_origin_lon')

        # Internal variables
        self.crs_wgs84 = CRS.from_epsg(4326)  # WGS 84 (GPS coordinates)
        self.crs_utm = CRS.from_epsg(25835)   # UTM Zone 35N
        self.utm_projection = Proj(self.crs_utm)

        # Create the coordinate transformer from WGS84 to UTM
        self.transformer = Transformer.from_crs(self.crs_wgs84, self.crs_utm)

        # Transform the origin point (Delta building coordinates)
        self.origin_x, self.origin_y = self.transformer.transform(utm_origin_lat, utm_origin_lon)

        # Subscribers
        rospy.Subscriber('/novatel/oem7/inspva', INSPVA, self.transform_coordinates)

        # Publishers
        self.current_pose_pub = rospy.Publisher('current_pose', PoseStamped, queue_size=10)
        self.current_velocity_pub = rospy.Publisher('current_velocity', TwistStamped, queue_size=10)
        self.br = TransformBroadcaster()

    def convert_azimuth_to_yaw(self, azimuth):
        """
        Converts azimuth to yaw. Azimuth is CW angle from the North. Yaw is CCW angle from the East.
        :param azimuth: azimuth in radians
        :return: yaw in radians
        """
        yaw = -azimuth + math.pi / 2
        if yaw > 2 * math.pi:
            yaw = yaw - 2 * math.pi
        elif yaw < 0:
            yaw += 2 * math.pi

        return yaw

    def transform_coordinates(self, msg):
        # Transform the GPS coordinates to UTM
        x, y = self.transformer.transform(msg.latitude, msg.longitude)

        # Subtract the origin point to get local coordinates
        local_x = x - self.origin_x
        local_y = y - self.origin_y

        # Calculate the position.z (subtract undulation from the height)
        local_z = msg.height - self.undulation

        # Calculate azimuth correction
        azimuth_correction = self.utm_projection.get_factors(msg.longitude, msg.latitude).meridian_convergence
        corrected_azimuth = msg.azimuth * math.pi / 180.0 - azimuth_correction  # Convert azimuth to radians

        # Convert the corrected azimuth to yaw
        yaw = self.convert_azimuth_to_yaw(corrected_azimuth)

        # Convert yaw to quaternion
        quaternion = quaternion_from_euler(0, 0, yaw)
        orientation = Quaternion(*quaternion)

        # Create and populate the PoseStamped message
        current_pose_msg = PoseStamped()
        current_pose_msg.header.stamp = rospy.Time.now()  # Using current time
        current_pose_msg.header.frame_id = "map"

        current_pose_msg.pose.position.x = local_x
        current_pose_msg.pose.position.y = local_y
        current_pose_msg.pose.position.z = local_z

        current_pose_msg.pose.orientation = orientation

        # Publish current pose
        self.current_pose_pub.publish(current_pose_msg)

        # Calculate velocity as the norm of north_velocity and east_velocity
        velocity = math.sqrt(msg.north_velocity**2 + msg.east_velocity**2)

        # Create and populate the TwistStamped message for velocity
        current_velocity_msg = TwistStamped()
        current_velocity_msg.header.stamp = rospy.Time.now()  # Use current time
        current_velocity_msg.header.frame_id = "base_link"

        # Set the velocity in the x direction (forward direction)
        current_velocity_msg.twist.linear.x = velocity
        current_velocity_msg.twist.linear.y = 0.0
        current_velocity_msg.twist.linear.z = 0.0

        # Set angular velocity to zero since we are not considering it here
        current_velocity_msg.twist.angular.x = 0.0
        current_velocity_msg.twist.angular.y = 0.0
        current_velocity_msg.twist.angular.z = 0.0

        # Publish current velocity
        self.current_velocity_pub.publish(current_velocity_msg)

        # Create and publish transform
        t = TransformStamped()
        t.header.stamp = current_pose_msg.header.stamp  # Use the timestamp from the pose message
        t.header.frame_id = "map"
        t.child_frame_id = "base_link"
        t.transform.translation.x = local_x
        t.transform.translation.y = local_y
        t.transform.translation.z = local_z
        t.transform.rotation = current_pose_msg.pose.orientation  # Use the same orientation as current_pose

        # Publish the transform
        self.br.sendTransform(t)

    def run(self):
        rospy.spin()

if __name__ == '__main__':
    rospy.init_node('localizer')
    node = Localizer()
    node.run()
